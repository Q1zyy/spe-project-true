...
scada.scheme.TableRenderer.prototype.createDom = function (component, renderContext) {
    ...
    var divComp = $("<div id='comp" + component.id + "' class='basic-table'></div>");
    this.prepareComponent(divComp, component, false, true);
    this.setBackColor(divComp, props.HeaderColor);
    this.bindAction(divComp, component, renderContext);

    // Render the table cells and rows here
    // This is just a basic example; you'd likely have nested loops for rows and cells.
    var divRow = $("<div class='table-row'></div>").appendTo(divComp);
    var divCell = $("<div class='table-cell'></div>").appendTo(divRow);
    // ... and so on ...

    component.dom = divComp;
};

scada.scheme.TableRenderer.prototype.updateData = function (component, renderContext) {
    var props = component.props;
    ...
    if (props.InCnlNum > 0) {
        ...
        // Define header color according to the channel status
        if (headerColor === this.STATUS_COLOR) {
            headerColor = cnlDataExt.Color;
        }

        // Define header color according to the table conditions and channel value
        if (cnlDataExt.Stat > 0 && props.Conditions) {
            var cnlVal = cnlDataExt.Val;

            for (var cond of props.Conditions) {
                if (scada.scheme.calc.conditionSatisfied(cond, cnlVal)) {
                    headerColor = cond.Color;
                    break;
                }
            }
        }
        ...
        // Apply header color
        divComp.css("background-color", headerColor);
    }
};*/
