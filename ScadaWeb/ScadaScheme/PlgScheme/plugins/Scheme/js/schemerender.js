...
scada.scheme.ChartRenderer.prototype.createDom = function (component, renderContext) {
    console.log("createDom called for component ID:", component.id);
    var props = component.props;
    console.log("Component properties:", props);

    if (!props.DataPoints || props.DataPoints.length === 0) {
        console.warn("No data points provided for the chart.");
        return;
    }

    // Отступы для графика
    var margin = { top: 15, right: 15, bottom: 15, left: 15 };
    var graphWidth = props.Width - margin.left - margin.right;
    var graphHeight = props.Height - margin.top - margin.bottom;

    // Смещение для осей
    var axisOffset = 5;

    // Увеличиваем верхний отступ для заголовка
    var increasedMarginTop = margin.top + 20;

    // Get the min and max values for scaling
    var minX = Math.min(...props.DataPoints.map(p => p.XValue));
    var maxX = Math.max(...props.DataPoints.map(p => p.XValue));
    var minY = Math.min(...props.DataPoints.map(p => p.YValue));
    var maxY = Math.max(...props.DataPoints.map(p => p.YValue));

    // Scale the points to fit the chart within the margins and consider the axis offset
    var scaleX = (props.Width - margin.left - margin.right - axisOffset) / (maxX - minX);
    var scaleY = (props.Height - increasedMarginTop - margin.bottom - axisOffset) / (maxY - minY);

    // Create the SVG path
    var pathData = "M";
    props.DataPoints.forEach(function (point, index) {
        var x = margin.left + axisOffset + (point.XValue - minX) * scaleX;
        var y = props.Height - margin.bottom - axisOffset - ((point.YValue - minY) * scaleY);
        pathData += x + " " + y + (index !== props.DataPoints.length - 1 ? " L " : "");
        console.log("Mapped point:", x, y);
    });

    var svgNS = "http://www.w3.org/2000/svg";
    var svgElem = document.createElementNS(svgNS, "svg");
    svgElem.setAttribute("width", props.Width);
    svgElem.setAttribute("height", props.Height);

    var pathElem = document.createElementNS(svgNS, "path");
    pathElem.setAttribute("d", pathData);
    pathElem.setAttribute("stroke", props.ChartColor);
    pathElem.setAttribute("fill", "none");
    svgElem.appendChild(pathElem);

    // Увеличиваем верхний отступ для заголовка
    var increasedMarginTop = margin.top + 7;

    // Добавление осей
    var xAxisElem = document.createElementNS(svgNS, "line");
    xAxisElem.setAttribute("x1", margin.left);
    xAxisElem.setAttribute("y1", props.Height - margin.bottom);
    xAxisElem.setAttribute("x2", props.Width - margin.right);
    xAxisElem.setAttribute("y2", props.Height - margin.bottom);
    xAxisElem.setAttribute("stroke", "black");
    svgElem.appendChild(xAxisElem);
    console.log("Added X-axis:", xAxisElem);

    var yAxisElem = document.createElementNS(svgNS, "line");
    yAxisElem.setAttribute("x1", margin.left);
    yAxisElem.setAttribute("y1", increasedMarginTop);
    yAxisElem.setAttribute("x2", margin.left);
    yAxisElem.setAttribute("y2", props.Height - margin.bottom);
    yAxisElem.setAttribute("stroke", "black");
    svgElem.appendChild(yAxisElem);
    console.log("Added Y-axis:", yAxisElem);

    // Добавление заголовков
    var chartTitleElem = document.createElementNS(svgNS, "text");
    chartTitleElem.setAttribute("x", props.Width / 2);
    chartTitleElem.setAttribute("y", increasedMarginTop / 2);
    chartTitleElem.setAttribute("text-anchor", "middle");
    chartTitleElem.textContent = props.ChartTitle;
    svgElem.appendChild(chartTitleElem);

    var xAxisTitleElem = document.createElementNS(svgNS, "text");
    xAxisTitleElem.setAttribute("x", props.Width / 2);
    xAxisTitleElem.setAttribute("y", props.Height - 5);
    xAxisTitleElem.setAttribute("text-anchor", "middle");
    xAxisTitleElem.textContent = props.XAxisTitle;
    svgElem.appendChild(xAxisTitleElem);

    var yAxisTitleElem = document.createElementNS(svgNS, "text");
    yAxisTitleElem.setAttribute("x", 5);
    yAxisTitleElem.setAttribute("y", props.Height / 2);
    yAxisTitleElem.setAttribute("writing-mode", "tb");
    yAxisTitleElem.setAttribute("text-anchor", "middle");
    yAxisTitleElem.setAttribute("dominant-baseline", "middle");
    yAxisTitleElem.textContent = props.YAxisTitle;
    svgElem.appendChild(yAxisTitleElem);

    component.dom = $(svgElem);
    console.log("Chart SVG element created:", svgElem);

    if (component.onDataPointsChanged) {
        component.onDataPointsChanged(() => {
            this.createDom(component, renderContext);
        });
    }
}

scada.scheme.ChartRenderer.prototype.bindEvents = function (component) {
    component.onDataPointsChanged = function (callback) {
        component.DataPointsChanged = callback;
    };
};

scada.scheme.ChartRenderer.prototype.setSize = function (component, width, height) {
    console.log("setSize called with width:", width, "and height:", height);
    component.props.Size = { Width: width, Height: height };
    var chart = component.dom;
    chart.attr("width", width);
    chart.attr("height", height);
};

scada.scheme.ChartRenderer.prototype.allowResizing = function (component) {
    console.log("allowResizing called for component ID:", component.id);
    return true; // allow resizing for all charts
};
...
// Table renderer type extends scada.scheme.ComponentRenderer
scada.scheme.TableRenderer = function () {
    scada.scheme.ComponentRenderer.call(this);
    console.log("TableRenderer constructor called.");
};
...
scada.scheme.TableRenderer.prototype.createDom = function (component, renderContext) {
    ...
    if (!props.Cells || props.Cells.length === 0) {
        console.warn("No cells data provided for the table.");
        return;
    }

    // Determine the size of the table based on Cells
    var maxRows = 0;
    var maxCols = 0;
    props.Cells.forEach(function (cell) {
        maxRows = Math.max(maxRows, cell.RowIndex + 1); // 0-based index
        maxCols = Math.max(maxCols, cell.ColIndex + 1); // 0-based index
    });

    // Create a matrix representing the table
    var matrix = [];
    for (var i = 0; i < maxRows; i++) {
        matrix.push(new Array(maxCols).fill(null));
    }
    ...
    var tableComp = $("<table id='comp" + component.id + "'></table>");
    tableComp.css("width", props.Size.Width + "px");
    tableComp.css("height", props.Size.Height + "px");
    tableComp.css("border-collapse", "collapse");
    tableComp.css("border", "1px solid black");

    var tbody = $("<tbody></tbody>");
    matrix.forEach(function (row, rowIndex) {
        var tr = $("<tr></tr>");

        // Применяем цвет к каждой строке на основе списка RowColors
        var rowColor = props.RowColors[rowIndex % props.RowColors.length];
        ...
        row.forEach(function (cell) {
            if (cell) {
                ...
                tr.append($("<td></td>").css("border", "1px solid black"));
            }
        });
        tbody.append(tr);
    });
    tableComp.append(tbody);
    component.dom = tableComp;
    console.log("Table DOM element created:", tableComp);
};



scada.scheme.TableRenderer.prototype.setSize = function (component, width, height) {
    console.log("setSize called with width:", width, "and height:", height);
    component.props.Size = { Width: width, Height: height };
    var table = component.dom;
    table.css("width", width + "px");
    table.css("height", height + "px");
};

scada.scheme.TableRenderer.prototype.allowResizing = function (component) {
    console.log("allowResizing called for component ID:", component.id);
    return true; // allow resizing for all tables
};
